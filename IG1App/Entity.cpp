...
void Tablero::render(Camera const & cam)
{
	if (mesh != nullptr) {
		for (int i = 0; i < 360; i= i+ (int) (360/n)) {
			...
			dmat4 t1 = translate(dmat4(1), glm::dvec3(r * cos(radians(incrAng+i)) / 2, (r / 2)*sin(radians(incrAng+i)), side * lado_caja / 2));
...
Drone::Drone(Chasis* chasis, Rotor* d, GLdouble R)
{
	...
	dmat4 t01 = translate(dmat4(1), leftBottom);
	d1->setModelMat(t01*rot);
	dvec3 leftTop = chasis->getLeftTopCorner();
	leftTop[1] += d2->getH() + 1.0;
	dmat4 t02 = translate(dmat4(1), leftTop);
	d2->setModelMat(t02*rot);
	dvec3 rightBottom = chasis->getRightBottomCorner();
	rightBottom[1] += d3->getH() + 1.0;
	dmat4 t03 = translate(dmat4(1), rightBottom);
	d3->setModelMat(t03*rot);
	dvec3 rightTop = chasis->getRightTopCorner();
	rightTop[1] += d4->getH() + 1.0;
	dmat4 t04 = translate(dmat4(1), rightTop);
	d4->setModelMat(t04*rot);
	...
	foco->setSpot(fvec3(0, -1, 0), 80.0, 1);
}
//-------------------------------------------------------------------------
